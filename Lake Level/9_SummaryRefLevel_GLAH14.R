...
GLAH14Level <- read.csv('E:\\Lake Volume\\Lake Level\\GLAH14_Ref_Compare\\GLAH14_Level.csv')
...
for(i in seq(1,length(fileNames))){
  ...
  # Row data
  selectIndex <- which(GLAH14Level$GLWD_ID == tempID)
  rowLakeLevelData <- GLAH14Level[selectIndex,]
...
write.csv(resultMatrix,file = 'E:\\Lake Volume\\Lake Level\\GLAH14_Ref_Compare\\Ref_Level.csv',row.names = FALSE)