rm(list = ls())
library(ggplot2)
setwd('E:\\Lake Volume\\0_Article_History\\MS_v4\\Figs\\Fig_2\\Fig_2a_2b')
...
dataMatrix <- read.csv('DVolume_Basin_Popu_100.csv')
...
write.csv(dataMatrix,file = 'DVolume_Basin_Popu_100_Draw.csv',row.names = FALSE)
...
Fig_2a <- ggplot()+
  ...
  theme_custom()
pdf('Fig_2a.pdf',width = 3.54,height = 2)
print(Fig_2a)
dev.off()
...
Fig_2b <- ggplot()+
  ...
  ylab('Number of basins')+
  theme_custom()
pdf('Fig_2b.pdf',width = 3.54,height = 2)
print(Fig_2b)
dev.off()