rm(list = ls())
library(ggplot2)
setwd('E:\\Lake Volume\\0_Article_History\\MS_v4\\Figs\\Fig_2\\Fig_2e_2f')
...
dataMatrix <- read.csv('DVolume_Basin_Popu_050.csv')
...
write.csv(dataMatrix,file = 'DVolume_Basin_Popu_050_Draw.csv',row.names = FALSE)
...
Fig_2e <- ggplot()+
  ...
  theme_custom()
pdf('Fig_2e.pdf',width = 3.54,height = 2)
print(Fig_2e)
dev.off()
...
Fig_2f <- ggplot()+
  ...
  ylab('Number of basins')+
  theme_custom()
pdf('Fig_2f.pdf',width = 3.54,height = 2)
print(Fig_2f)
dev.off()