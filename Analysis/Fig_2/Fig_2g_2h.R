rm(list = ls())
library(ggplot2)
setwd('E:\\Lake Volume\\0_Article_History\\MS_v4\\Figs\\Fig_2\\Fig_2g_2h')
...
dataMatrix <- read.csv('DVolume_Basin_Popu_025.csv')
...
write.csv(dataMatrix,file = 'DVolume_Basin_Popu_025_Draw.csv',row.names = FALSE)
...
breakSeq <- c(-500000,-100000,-50000,-10000,-5000,-1000,-500,-100,0,100,500,1000,5000,10000,50000,100000,500000)
for(i in seq(1,length(breakSeq))){
  if(i == 1){
...
Fig_2g <- ggplot()+
  ...
  theme_custom()
pdf('Fig_2g.pdf',width = 3.54,height = 2)
print(Fig_2g)
dev.off()
...
Fig_2h <- ggplot()+
  ...
  scale_y_continuous(limits = c(0,57),breaks = seq(0,50,10),labels = c('0','10','20','30','40','50'),expand = expansion(mult = 0.03))+
  scale_fill_manual(values = c('#d4232d','#2f7cb2'))+
  xlab('Change in lake volume per capita')+
  ylab('Number of basins')+
  theme_custom()
pdf('Fig_2h.pdf',width = 3.54,height = 2)
print(Fig_2h)
dev.off()